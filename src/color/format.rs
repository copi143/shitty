//! 颜色格式定义。包含所有非默认的颜色类型（RGBA/BGRA/ARGB/ABGR、RGB565 等）。
//!
//! 默认使用 `color` 宏生成的 `Color` 类型；启用 `color-formats` feature 后，
//! 会额外生成 `RGBA`/`BGRA`/`ARGB`/`ABGR`（u8/u16/f32 变体）、`RGB`/`BGR`、
//! `RGB565`/`BGR565` 等类型以及对应的 `From`/算术运算实现。
//!
//! ---
//!
//! Color format definitions. Contains all non-default color types
//! (RGBA/BGRA/ARGB/ABGR, RGB565, etc.).
//!
//! The `Color` type is always available via the `color` macro. When the
//! `color-formats` feature is enabled, additional types like `RGBA`/`BGRA`/`ARGB`/`ABGR`
//! (u8/u16/f32 variants), `RGB`/`BGR`, `RGB565`/`BGR565`, and their corresponding
//! `From`/arithmetic impls are also generated.
...
macro_rules! color {
    ...
    (^inner1^ $(#[$doc:meta])* $name:ident, $type:ty, $align:literal) => {
        ...
        #[repr(C, align($align))]
        pub struct $name {
            #[cfg(feature = "color-a-first")]         pub a: $type,
            #[cfg(not(feature = "color-b-before-r"))] pub r: $type,
            #[cfg(feature = "color-b-before-r")]      pub b: $type,
                                                      pub g: $type,
            #[cfg(feature = "color-b-before-r")]      pub r: $type,
            #[cfg(not(feature = "color-b-before-r"))] pub b: $type,
            #[cfg(not(feature = "color-a-first"))]    pub a: $type,
        }
        ...
        static_assert!(core::mem::size_of::<Color>() == 4);
        impl $name {
            ...
            pub const fn from_u32(c: u32) -> Self {
                #[expect(unsafe_code)] unsafe { Self::from(core::mem::transmute::<u32, Color>(c)) }
            }
            pub const fn as_u32(&self) -> u32 {
                #[expect(unsafe_code)] unsafe { core::mem::transmute::<Color, u32>(Color::from(*self)) }
            }
        }
        const impl From<crate::color::AnsiRgb> for $name {
    ...
    (^inner2^ $(#[$doc:meta])* $name:ident, $type:ty, $align:literal) => {
        ...
        const impl From<Color> for $name {
        ...
        const impl From<$name> for Color {
...
#[cfg(not(feature = "color-formats"))]
from_slice_!(try u8 * 4; Color);

#[cfg(not(feature = "color-formats"))]
from_slice_!(u32; Color);
